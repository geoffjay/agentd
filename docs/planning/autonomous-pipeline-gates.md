...
This document defines exactly where the autonomous pipeline pauses for human input. Three categories of gates exist: operations that are always human, gates that are configurable (autonomous by default with a flag to require approval), and operations that are always autonomous.
...
|---|---|---|---|
| PR auto-merge | `allow` | `conductor.auto_merge` | Conductor posts intent to engineering and waits for a `:+1:` reaction before merging |
...
The always-human list must also be reflected in `CLAUDE.md` so that any agent invoking commands directly (outside a formal workflow) respects these boundaries. Add the following section:
