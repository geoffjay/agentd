...
use kanagawa::{AnsiColor, ColorName, PALETTE, Rgb};

fn main() {
    let latte_teal = PALETTE.latte.colors.teal;
    let Rgb { r, g, b } = latte_teal.rgb;
    println!("Latte's {} is {}, which is rgb({r}, {g}, {b})", latte_teal.name, latte_teal.hex);
    ...
    println!("Mocha's {} is {}, which is rgb({r}, {g}, {b})", mocha_teal.name, mocha_teal.hex);
    ...
    for AnsiColor { name, rgb, hsl, code, hex } in &mocha.ansi_colors {
        println!(
