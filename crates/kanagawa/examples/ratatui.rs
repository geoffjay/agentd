...
fn main() -> io::Result<()> {
    ...
    for theme in &PALETTE {
        terminal.insert_before(8, |buf| {
            let analogous: Vec<Span> = theme
                .colors
                .into_iter()
                .filter(|c| c.accent)
                .map(|c| "██".fg(*c)) // fg accepts any type that implements Into<Color>
                .collect::<Vec<Span>>();
            let monochromatic: Vec<Span> = theme
                .colors
                .into_iter()
                .filter(|c| !c.accent)
                .map(|c| "██".fg(*c)) // fg accepts any type that implements Into<Color>
                .collect();
            let ansi_normals: Vec<Span> = theme
                .ansi_colors
                .into_iter()
                .filter(|c| c.code < 8)
                .map(|c| "██".fg(*c)) // fg accepts any type that implements Into<Color>
                .collect::<Vec<Span>>();
            let ansi_brights: Vec<Span> = theme
                .ansi_colors
                .into_iter()
                .filter(|c| c.code >= 8)
                .map(|c| "██".fg(*c)) // fg accepts any type that implements Into<Color>
            ...
            Paragraph::new(theme.name.to_string()).render(Rect::new(0, 0, width, 1), buf);
            ...
            Paragraph::new(format!("{} ANSI", theme.name)).render(Rect::new(0, 4, width, 1), buf);
