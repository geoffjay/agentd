...
fn titlecase<S: AsRef<str>>(s: S) -> String {
    ...
    chars.next().map_or_else(String::new, |first| first.to_uppercase().to_string() + chars.as_str())
}
...
fn make_color_entry(identifier: &str, color: &Color) -> TokenStream {
    ...
    let Color { order, accent, rgb: Rgb { r, g, b }, hsl: Hsl { h, s, l }, .. } = color;
...
fn make_ansi_color_entry(identifier: &str, ansi_color: &AnsiColor) -> TokenStream {
    let ident = format_ident!("{identifier}");
    let AnsiColor { name, code, rgb: Rgb { r, g, b }, hsl: Hsl { h, s, l } } = ansi_color;
