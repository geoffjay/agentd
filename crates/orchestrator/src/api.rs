...
async fn get_agent_conversation(
    ...
    // Session filter: applied in memory since ConversationQuery doesn't have a
    // session field (the storage layer orders by created_at, not session).
    if let Some(session) = query.session {
        events.retain(|e| e.session_number == session);
    }
