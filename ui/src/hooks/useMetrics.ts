...
import type { HttpMetricsSummary, ParsedMetrics } from "./usePrometheusParser";
import { extractHttpMetrics, parsePrometheusText } from "./usePrometheusParser";
...
const SERVICE_ENDPOINTS = [
	{
		...
		name: "Orchestrator",
		port: 17006,
		url: () => serviceConfig.orchestratorServiceUrl,
	},
	{
		key: "notify",
		name: "Notify",
		port: 17004,
		url: () => serviceConfig.notifyServiceUrl,
	},
	{
		key: "ask",
		name: "Ask",
		port: 17001,
		url: () => serviceConfig.askServiceUrl,
	},
];

const EMPTY_AGENT_COUNTS: AgentStatusCounts = {
	running: 0,
	pending: 0,
	stopped: 0,
	failed: 0,
};
const EMPTY_NOTIF_COUNTS: NotificationCounts = {
	Low: 0,
	Normal: 0,
	High: 0,
	Urgent: 0,
...
async function fetchPrometheusMetrics(
	baseUrl: string,
): Promise<ParsedMetrics | undefined> {
	try {
...
export function useMetrics(
	initialInterval: RefreshInterval = 30_000,
): UseMetricsResult {
	const [agentCounts, setAgentCounts] =
		useState<AgentStatusCounts>(EMPTY_AGENT_COUNTS);
	const [notifCounts, setNotifCounts] =
		useState<NotificationCounts>(EMPTY_NOTIF_COUNTS);
	...
	const [refreshInterval, setRefreshInterval] =
		useState<RefreshInterval>(initialInterval);
	...
	const fetchAll = useCallback(async () => {
		try {
			...
			if (agentResult.status === "fulfilled") {
				...
				historyRef.current = [...historyRef.current, point].slice(
					-MAX_HISTORY_POINTS,
				);
			...
			if (
				...
				const nCounts: NotificationCounts = {
					Low: 0,
					Normal: 0,
					High: 0,
					Urgent: 0,
			...
			const svcData: ServiceMetricsData[] = SERVICE_ENDPOINTS.map((svc, i) => {
				...
				const raw =
					promResult?.status === "fulfilled" ? promResult.value : undefined;
				return {
					key: svc.key,
					name: svc.name,
					port: svc.port,
					http: raw
						? extractHttpMetrics(raw)
						: { requestsTotal: 0, errorsTotal: 0, errorRate: 0 },
					raw,
