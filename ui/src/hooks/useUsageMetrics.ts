		...
		return {
			...
			stats: {
				...
				cumulative: {
					...
					input_tokens:
						entry.stats.cumulative.input_tokens + snapshot.input_tokens,
					output_tokens:
						entry.stats.cumulative.output_tokens + snapshot.output_tokens,
					cache_read_input_tokens:
						entry.stats.cumulative.cache_read_input_tokens +
					...
					total_cost_usd:
						entry.stats.cumulative.total_cost_usd + snapshot.total_cost_usd,
					num_turns: entry.stats.cumulative.num_turns + snapshot.num_turns,
					duration_ms:
						entry.stats.cumulative.duration_ms + snapshot.duration_ms,
					duration_api_ms:
						entry.stats.cumulative.duration_api_ms + snapshot.duration_api_ms,
...
export function useUsageMetrics(
	...
	const fetchAll = useCallback(async () => {
		try {
			...
			if (mountedRef.current) {
				setError(
					err instanceof Error ? err.message : "Failed to fetch usage metrics",
				);
			}
		} finally {
			if (mountedRef.current) {
