...
function computeAggregateUsage(
	usageList: AgentUsageStats[],
	...
	const cacheHitPercent =
		cacheTotal > 0 ? (totalCacheRead / cacheTotal) * 100 : 0;
...
export function useAgentSummary(): UseAgentSummaryResult {
	...
	const [aggregateUsage, setAggregateUsage] =
	...
	const fetch = useCallback(async () => {
		...
		try {
			...
			const newCounts: AgentStatusCounts = {
				running: 0,
				pending: 0,
				stopped: 0,
				failed: 0,
			...
			setAggregateUsage(
				usageList.length > 0 ? computeAggregateUsage(usageList) : null,
			);
		} catch (err) {
