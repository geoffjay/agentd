	...
	if (field === "name") {
		...
		const aRatio = aStats
			? aStats.cache_read_input_tokens /
				(aStats.cache_read_input_tokens +
					aStats.cache_creation_input_tokens +
					aStats.input_tokens || 1)
			: 0;
		const bRatio = bStats
			? bStats.cache_read_input_tokens /
				(bStats.cache_read_input_tokens +
					bStats.cache_creation_input_tokens +
					bStats.input_tokens || 1)
			: 0;
...
export function useAgents({
	...
	const [usageMap, setUsageMap] = useState<Map<string, AgentUsageStats>>(
		new Map(),
	);
			...
			try {
				...
				const usageResults = await Promise.allSettled(
					result.items.map((agent) =>
						orchestratorClient.getAgentUsage(agent.id),
					),
				);
	...
	const sorted = [...filtered].sort((a, b) =>
		compareAgents(a, b, sortBy, sortDir, usageMap),
	);
