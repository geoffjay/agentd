...
function UnavailableFallback({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 p-6 text-center">
      ...
      <div>
        <p className="text-sm font-medium text-gray-400">PTY streaming not available</p>
        <p className="mt-1 text-xs text-gray-600">
          This agent&apos;s backend does not support PTY streaming. Terminal output is only
          available with a PTY-backed session (e.g. the wrap service in PTY mode).
          Agents running on tmux or Docker backends use the{' '}
          <strong className="text-gray-400">Logs</strong> tab instead.
        </p>
      </div>
      <button
        type="button"
...
export function AgentTerminal({
  ...
  return (
    ...
    >
      ...
      {status === 'unavailable' && <UnavailableFallback onRetry={handleRetry} />}
    </div>
  )
}
