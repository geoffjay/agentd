...
function makeUsageStats(
	overrides?: Partial<AgentUsageStats["cumulative"]>,
): AgentUsageStats {
	return {
		agent_id: "",
		cumulative: {
...
describe("AgentTable usage columns", () => {
	...
	it("renders Cache Hit column header with sort button", () => {
		...
		render(<AgentTable {...defaultProps} agents={agents} />, { wrapper });
		expect(
			screen.getByRole("button", { name: /cache hit/i }),
		).toBeInTheDocument();
	});
	...
	it('calls onSort with "cost" when Cost header is clicked', () => {
		const onSort = vi.fn();
		const agents = makeAgentList(1);
		render(<AgentTable {...defaultProps} agents={agents} onSort={onSort} />, {
			wrapper,
		});
	...
	it("displays formatted cost when usageMap is provided", () => {
		...
		render(
			<AgentTable {...defaultProps} agents={agents} usageMap={usageMap} />,
			{ wrapper },
		);
	...
	it("displays formatted tokens when usageMap is provided", () => {
		...
		usageMap.set(
			agents[0].id,
			makeUsageStats({ input_tokens: 15000, output_tokens: 5000 }),
		);
		render(
			<AgentTable {...defaultProps} agents={agents} usageMap={usageMap} />,
			{ wrapper },
