...
import {
	USAGE_STATS,
	USAGE_STATS_NO_SESSION,
	USAGE_STATS_ZERO,
} from "@/test/fixtures/usage";
...
describe("AgentUsagePanel", () => {
	...
	it("displays current session stat cards with correct aria labels", () => {
		...
		// Cache creation: 100
		expect(
			screen.getByLabelText(/Cache creation tokens: 100/),
		).toBeInTheDocument();
	});
	...
	it("displays wall clock duration", () => {
		...
		expect(
			screen.getByLabelText(/Wall clock duration: 12\.0s/),
		).toBeInTheDocument();
	});
	...
	it("shows auto-clear threshold progress when configured", () => {
		render(<AgentUsagePanel usage={USAGE_STATS} autoClearThreshold={0.1} />);
		expect(screen.getByText("Auto-clear Threshold")).toBeInTheDocument();
		expect(
			screen.getByLabelText(/Auto-clear threshold progress/),
		).toBeInTheDocument();
	});
	...
	it("shows warning when approaching threshold", () => {
		...
		expect(
			screen.getByText("Approaching auto-clear threshold"),
		).toBeInTheDocument();
	});
	...
	it("does not show warning when far from threshold", () => {
		...
		expect(
			screen.queryByText("Approaching auto-clear threshold"),
	...
	it("does not show auto-clear when no active session", () => {
		render(
			<AgentUsagePanel
				usage={USAGE_STATS_NO_SESSION}
				autoClearThreshold={0.1}
			/>,
		);
		expect(screen.queryByText("Auto-clear Threshold")).not.toBeInTheDocument();
	});
