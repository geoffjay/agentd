...
describe('AgentPolicyEditor', () => {
  it('renders policy type dropdown', () => {
    render(<AgentPolicyEditor policy={{ type: 'AllowAll' }} onSave={vi.fn()} />)
  ...
  it('shows tool list input when AllowList is selected', () => {
    render(<AgentPolicyEditor policy={{ type: 'AllowList', tools: ['bash'] }} onSave={vi.fn()} />)
  ...
  it('shows tool list input when DenyList is selected', () => {
    render(
      <AgentPolicyEditor policy={{ type: 'DenyList', tools: ['rm', 'dd'] }} onSave={vi.fn()} />,
    )
  ...
  it('calls onSave with correct AllowAll policy', async () => {
    ...
    await waitFor(() => expect(onSave).toHaveBeenCalledWith({ type: 'AllowAll' }))
  })

  it('calls onSave with parsed tools for AllowList', async () => {
    const onSave = vi.fn().mockResolvedValue(undefined)
    render(<AgentPolicyEditor policy={{ type: 'AllowList', tools: [] }} onSave={onSave} />)
    // Change tools
  ...
  it('shows "Saving…" label when saving=true', () => {
    render(<AgentPolicyEditor policy={{ type: 'AllowAll' }} saving onSave={vi.fn()} />)
