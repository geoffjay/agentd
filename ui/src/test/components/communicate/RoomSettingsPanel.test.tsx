...
import { makeParticipantList, makeRoom } from "@/test/mocks/factories";
...
function renderPanel(
	overrides?: Partial<Parameters<typeof RoomSettingsPanel>[0]>,
) {
	const room = makeRoom({
		name: "test-room",
		topic: "Test topic",
...
describe("RoomSettingsPanel", () => {
	...
	it("calls onRoomUpdated after saving topic and description", async () => {
		...
		server.use(http.put(`${BASE}/rooms/:id`, () => HttpResponse.json(updated)));
	...
	it("loads and displays participants", async () => {
		...
		await waitFor(() =>
			expect(
				screen.getByText(participants[0].display_name),
			).toBeInTheDocument(),
		);
