...
describe("MessageInput", () => {
	it("renders textarea and send button when participant", () => {
		render(
			<MessageInput onSend={vi.fn()} isParticipant={true} onJoin={vi.fn()} />,
		);
		expect(
	...
	it("shows join prompt when not a participant", () => {
		render(
			<MessageInput onSend={vi.fn()} isParticipant={false} onJoin={vi.fn()} />,
		);
	...
	it("calls onSend with trimmed text when send button is clicked", async () => {
		...
		render(
			<MessageInput onSend={onSend} isParticipant={true} onJoin={vi.fn()} />,
		);
