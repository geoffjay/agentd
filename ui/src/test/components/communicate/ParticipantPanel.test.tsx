...
describe("ParticipantPanel", () => {
	...
	it("merges realtime joined participants", async () => {
		const existing = makeParticipantList(1, {
			room_id: ROOM_ID,
			kind: "agent",
		});
		...
		render(
			<ParticipantPanel roomId={ROOM_ID} realtimeParticipants={[newJoiner]} />,
		);
	...
	it("hides participants who left", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText(participants[1].display_name),
			).toBeInTheDocument();
		});

		expect(
			screen.queryByText(participants[0].display_name),
