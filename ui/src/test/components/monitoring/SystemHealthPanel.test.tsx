	...
	const defaults: ServiceMetricsData[] = [
		{
			...
			name: "Orchestrator",
			port: 17006,
			http: { requestsTotal: 100, errorsTotal: 0, errorRate: 0 },
			reachable: true,
		},
		{
			key: "notify",
			name: "Notify",
			port: 17004,
			http: { requestsTotal: 50, errorsTotal: 0, errorRate: 0 },
			reachable: true,
		},
		{
			key: "ask",
			name: "Ask",
			port: 17001,
			http: { requestsTotal: 20, errorsTotal: 0, errorRate: 0 },
			reachable: false,
		},
	];
...
describe("SystemHealthPanel", () => {
	...
	it("shows loading skeletons when loading=true", () => {
		const { container } = render(
			<SystemHealthPanel serviceMetrics={[]} loading />,
		);
