...
// Mock Nivo charts
vi.mock("@nivo/bar", () => ({
	ResponsiveBar: ({ ariaLabel }: { ariaLabel: string }) => (
		<div role="img" aria-label={ariaLabel} />
	),
}));
vi.mock("@nivo/pie", () => ({
	ResponsivePie: ({ ariaLabel }: { ariaLabel: string }) => (
		<div role="img" aria-label={ariaLabel} />
	),
}));
vi.mock("@nivo/line", () => ({
...
describe("AgentActivityChart", () => {
	...
	it("defaults to bar chart view", () => {
		render(<AgentActivityChart counts={COUNTS} timeSeries={TIME_SERIES} />);
		expect(
			screen.getByLabelText("Agent status distribution bar chart"),
		).toBeTruthy();
	});
	...
	it("shows loading skeleton when loading=true", () => {
		const { container } = render(
			<AgentActivityChart counts={COUNTS} timeSeries={[]} loading />,
		);
