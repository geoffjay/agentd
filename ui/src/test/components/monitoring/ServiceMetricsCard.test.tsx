...
function makeServiceData(
	overrides: Partial<ServiceMetricsData> = {},
): ServiceMetricsData {
	return {
		key: "orchestrator",
		name: "Orchestrator",
...
describe("ServiceMetricsCard", () => {
	...
	it("shows request count when reachable", () => {
		render(
			<ServiceMetricsCard
				data={makeServiceData({
					http: { requestsTotal: 1500, errorsTotal: 0, errorRate: 0 },
				})}
			/>,
		);
	...
	it("shows error count when there are errors", () => {
		render(
			<ServiceMetricsCard
				data={makeServiceData({
					http: { requestsTotal: 100, errorsTotal: 3, errorRate: 0.03 },
				})}
			/>,
		);
	...
	it("renders loading skeleton when loading=true", () => {
		const { container } = render(
			<ServiceMetricsCard data={makeServiceData()} loading />,
		);
	...
	it("labels slow response correctly", () => {
		render(
			<ServiceMetricsCard data={makeServiceData()} responseTimeMs={600} />,
		);
	...
	it("labels OK response correctly", () => {
		render(
			<ServiceMetricsCard data={makeServiceData()} responseTimeMs={200} />,
		);
