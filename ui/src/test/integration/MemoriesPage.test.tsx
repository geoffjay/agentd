...
describe("MemoriesPage (MSW integration)", () => {
	...
	it("displays loading skeletons while fetching", () => {
		...
		server.use(http.get(`${BASE}/memories`, () => new Promise(() => {})));
	...
	it("renders memory cards with MSW data", async () => {
		...
		await waitFor(() => {
			expect(
				screen.getByText("Deployment runbook for production"),
			).toBeInTheDocument();
		});
		expect(
			screen.getByText("API rate limit configuration"),
		).toBeInTheDocument();
	});
	...
	it("renders filter controls for type, visibility, and content search", async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.getByLabelText("Search memory content"),
			).toBeInTheDocument();
		});
	});

	it("filters by memory type via the type dropdown", async () => {
		const infoMem = makeMemory({
			content: "Info content",
			type: "information",
		});
		const questionMem = makeMemory({
	...
	it("paginates through memory pages", async () => {
		...
		const nextButton =
			screen.queryByLabelText("Next page") ?? screen.queryByText("Next");
		if (nextButton) {
	...
	it("opens create dialog, validates, submits, and shows toast", async () => {
		...
		await waitFor(() => {
			...
			expect(
				screen.getByPlaceholderText("Enter the memory content…"),
			).toBeInTheDocument();
		});
		...
		await waitFor(() => {
			...
			expect(
				screen.queryByPlaceholderText("Enter the memory content…"),
	...
	it("switches to search mode and displays results", async () => {
		...
		const searchTab = screen.getByRole("button", {
			name: "Search",
			pressed: false,
		});
		...
		await waitFor(() => {
			...
			expect(
				screen.getByPlaceholderText(/semantic search across memories/i),
			).toBeInTheDocument();
		});
	});
