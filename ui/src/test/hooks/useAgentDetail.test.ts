...
function paginatedApprovals(
	items: PendingApproval[],
): PaginatedResponse<PendingApproval> {
...
describe("useAgentDetail", () => {
	it("fetches agent on mount", async () => {
		...
		const { result } = renderHook(() =>
			useAgentDetail("agent-1", { refreshInterval: 0 }),
		);
	...
	it("sets error when agent fetch fails", async () => {
		...
		const { result } = renderHook(() =>
			useAgentDetail("bad-id", { refreshInterval: 0 }),
		);
	...
	it("fetches pending approvals on mount", async () => {
		...
		const { result } = renderHook(() =>
			useAgentDetail("agent-2", { refreshInterval: 0 }),
		);
	...
	it("updateModel updates local agent state", async () => {
		...
		const updatedAgent = {
			...agent,
			config: { ...agent.config, model: "new-model" },
		};
		server.use(
			...
			http.put(`${BASE}/agents/agent-3/model`, () =>
				HttpResponse.json(updatedAgent),
			),
		);

		const { result } = renderHook(() =>
			useAgentDetail("agent-3", { refreshInterval: 0 }),
		);
		await waitFor(() => expect(result.current.loading).toBe(false));
	...
	it("approveRequest removes approval from local state", async () => {
		...
		const { result } = renderHook(() =>
			useAgentDetail("agent-4", { refreshInterval: 0 }),
		);
		await waitFor(() => expect(result.current.approvalsLoading).toBe(false));
		expect(result.current.approvals).toHaveLength(1);
	...
	it("denyRequest removes approval from local state", async () => {
		...
		const { result } = renderHook(() =>
			useAgentDetail("agent-5", { refreshInterval: 0 }),
		);
		await waitFor(() => expect(result.current.approvalsLoading).toBe(false));
