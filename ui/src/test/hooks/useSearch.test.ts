...
describe("useSearch", () => {
	...
	it("finds notification results matching the query after debounce", async () => {
		...
		expect(result.current.results.notifications[0].title).toBe(
			"High memory alert",
		);
	});
	...
	it("filters quick actions matching the query", async () => {
		...
		expect(
			result.current.results.actions.some((a) => a.title.includes("Agent")),
		).toBe(true);
	});
	...
	it("persists recent searches to localStorage", () => {
		...
		const stored = JSON.parse(
			localStorage.getItem("agentd:recent-searches") ?? "[]",
		) as string[];
	...
	it("deduplicates recent searches", () => {
		...
		expect(
			result.current.recentSearches.filter((s) => s === "build-bot"),
		).toHaveLength(1);
	});
