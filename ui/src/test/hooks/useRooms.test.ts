...
describe("useRooms", () => {
	...
	it("loads rooms on mount", async () => {
		...
		vi.spyOn(communicateClient, "listRooms").mockResolvedValue(
			paginated(rooms),
		);
	...
	it("filters rooms by name search client-side", async () => {
		...
		expect(result.current.rooms.every((r) => r.name.includes("channel"))).toBe(
			true,
		);
	});

	it("sets error on fetch failure", async () => {
		vi.spyOn(communicateClient, "listRooms").mockRejectedValue(
	...
	it("refetch re-loads rooms", async () => {
		...
		const spy = vi
			.spyOn(communicateClient, "listRooms")
			.mockResolvedValue(paginated(rooms));
	...
	it("passes room_type filter to the API", async () => {
		...
		expect(spy).toHaveBeenCalledWith(
			expect.objectContaining({ room_type: "direct" }),
		);
	});
});
