...
describe("settingsStore", () => {
	...
	it("loadSettings merges saved values with defaults", () => {
		...
		expect(settings.services.notifyUrl).toBe(
			DEFAULT_SETTINGS.services.notifyUrl,
		);
	...
	it("resetSettings returns defaults and clears localStorage entry", () => {
		...
		expect(reloaded.services.orchestratorUrl).toBe(
			DEFAULT_SETTINGS.services.orchestratorUrl,
		);
	});
