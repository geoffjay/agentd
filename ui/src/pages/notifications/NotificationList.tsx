...
function NotificationListInner() {
	...
	const [filters, setFiltersState] = useState<NotificationFilters>({
		status:
			(searchParams.get("status") as NotificationFilters["status"]) ?? "All",
		priority:
			(searchParams.get("priority") as NotificationFilters["priority"]) ??
			"All",
		source:
			(searchParams.get("source") as NotificationFilters["source"]) ?? "All",
	});
	...
	const handleBulkDismiss = async () => {
		if (
			!window.confirm(
				`Dismiss ${selectedIds.length} notification${selectedIds.length !== 1 ? "s" : ""}?`,
			)
		)
			return;
	...
	const handleMarkAllViewed = async () => {
		...
		if (
			!window.confirm(
				`Mark ${pendingCount} notification${pendingCount !== 1 ? "s" : ""} as viewed?`,
			)
		)
			return;
		await markAllViewed();
