...
export function CommunicatePage() {
	...
	const [rightPanel, setRightPanel] = useState<"participants" | "settings">(
		"participants",
	);
	...
	useEffect(() => {
		...
		async function checkMembership() {
			...
			let offset = 0;
			while (!cancelled) {
				const res = await communicateClient.listParticipants(roomId, {
					limit,
					offset,
				});
	...
	const handleParticipantJoined = useCallback((participant: Participant) => {
		setRealtimeParticipants((prev) => {
			if (prev.some((p) => p.identifier === participant.identifier))
				return prev;
	...
	const handleParticipantLeft = useCallback(
		(_roomId: string, identifier: string) => {
			setLeftIdentifiers((prev) =>
	...
	return (
		<div
			className={`fixed inset-0 top-16 p-4 transition-all duration-300 ease-in-out ${sidebarOpen ? "lg:left-60" : "lg:left-16"}`}
		>
						...
						<>
							...
							<div className="flex h-12 shrink-0 items-center gap-3 border-b border-gray-800 px-4">
								...
								<button
									...
									onClick={() =>
										setRightPanel((p) =>
											p === "settings" ? "participants" : "settings",
										)
									}
